...
pub(super) fn draw_region_detail(ctx: &UiContext<'_>, rect: Rect, actions: &mut Vec<UiAction>) {
    ...
    // Drill-in: if a town of this region is open, the detail panel becomes a town
    // browser — a sub-menu of the region's towns with the selected one's detail
    // (GDD 10). Stale selections (a town of another region) fall through.
    if let Some(town_id) = ctx.selected_town {
        if ctx
            .world
            .settlements
            .iter()
            .any(|s| s.id == town_id && s.region_id == region.id)
        {
            draw_town_browser(ctx, rect, region, town_id, actions);
            return;
        }
    }
    ...
    // might folds in any War-artifact empowerment and the region's resident
    // heroes, so the shown number matches what conquest actually weighs.
    let gtext = &strings.genesis;
    ...
    draw_ui_text_ex(
        &fill(
            ...
            &[(
                "might",
                format!("{:.0}", region.might(conquest) + war_might + hero_might),
            )],
        ),
        content.x,
        y,
    ...
    // The towns line is clickable: it drills into the town browser (opened on the
    // region's first town), so a player can inspect any settlement in detail.
    if town_total > 0 {
        let row = Rect::new(content.x, y, content.w, 18.0);
        ...
        draw_ui_text_ex(
            &fill(&ui.settlements_line, &[("list", more(&towns, town_total))]),
            content.x,
            y + 13.0,
        ...
        if hovered && is_mouse_button_released(MouseButton::Left) {
            if let Some(first) = ctx
                ...
                .settlements
                .iter()
                .find(|s| s.region_id == region.id)
            {
                actions.push(UiAction::SelectTown(first.id.clone()));
            }
        }
        y += 18.0;
    }
    ...
    if lines.is_empty() && town_total == 0 {
        draw_ui_text_ex(
            &ui.no_holdings,
            content.x,
            y + 14.0,
...
/// The town browser: a sub-view of the selected region's holdings (GDD 10). The
/// left column lists every town in the region (clickable to switch), the right
/// column shows the chosen town's tier, population, prosperity, and the works
/// raised in it. A Back button closes it. Pure view — clicks return intents.
fn draw_town_browser(
    ctx: &UiContext<'_>,
    rect: Rect,
    region: &Region,
    town_id: &str,
