...
fn region_action(
    ...
    // Anti-grief (GDD 7.5): a region can absorb only so much divine nudging per
    // tick, summed across *every* deity. Reject a nudge that would overflow the
    // shared per-region budget before any favor is spent, so the richest god
    // can't simply out-spend the world — the saturation is visible to all, and a
    // rival can time its own push for the next tick.
    let magnitude = region.nudge_magnitude(&def, region_balance);
    if !region.nudge_fits(magnitude, region_balance) {
        report.warning(fill(
            &notes.region_saturated,
            &[("region", region.name.clone())],
        ));
        return;
    }
    let cost = region.action_cost(&def, region_balance);
    ...
    let region_name;
    {
        ...
        region.apply_action(&def, region_balance);
        region.record_nudge(magnitude);
